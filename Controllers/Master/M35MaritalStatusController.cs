...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M35MaritalStatusController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M35MaritalStatusController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult MaritalStatusOperation([FromBody] M35MaritalStatus model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M35_MaritalStatus_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@MaritalStatusName", (object?)model.MaritalStatusName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@StatusCode", (object?)model.StatusCode ?? DBNull.Value);

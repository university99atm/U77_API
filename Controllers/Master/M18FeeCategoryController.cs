...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M18FeeCategoryController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M18FeeCategoryController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult FeeCategoryOperation([FromBody] M18FeeCategory model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M18_FeeCategory_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@FeeCategoryName", (object?)model.FeeCategoryName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@FeeCategoryCode", (object?)model.FeeCategoryCode ?? DBNull.Value);

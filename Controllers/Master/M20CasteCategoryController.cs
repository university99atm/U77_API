...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M20CasteCategoryController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M20CasteCategoryController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult CasteCategoryOperation([FromBody] M20CasteCategory model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M20_CasteCategory_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@CategoryName", (object?)model.CategoryName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@CategoryCode", (object?)model.CategoryCode ?? DBNull.Value);

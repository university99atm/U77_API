...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M29StudentSiblingController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M29StudentSiblingController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult StudentSiblingOperation([FromBody] M29StudentSibling model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M29_StudentSibling_Operation]", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@RelationshipCode", (object?)model.RelationshipCode ?? DBNull.Value);

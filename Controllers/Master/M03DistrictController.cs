...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M03DistrictController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M03DistrictController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult DistrictOperation([FromBody] M03District model)
        {
            try
            {
                // 🔐 JWT Claims
                ...
                using (SqlCommand cmd = new SqlCommand(
                    "[dbo].[U77_Pro_M03_districtoperation]", con))
                {
                ...
                // ✅ Stored Procedure Response Handling
                if (dt.Rows.Count > 0)
                {
                    var row = dt.Rows[0];

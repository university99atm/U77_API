...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M40AddressTypeController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M40AddressTypeController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult AddressTypeOperation([FromBody] M40AddressType model)
        {
            try
            {
                /* ================= JWT ================= */
                ...
                using (SqlCommand cmd = new SqlCommand(
                    "[dbo].[U77_Pro_M40_AddressType_Operation]", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@AddressTypeCode",
                        (object?)model.AddressTypeCode ?? DBNull.Value);

                    cmd.Parameters.AddWithValue("@AddressTypeName",
                        (object?)model.AddressTypeName ?? DBNull.Value);

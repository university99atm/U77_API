...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M39StudentPhotoTypeController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M39StudentPhotoTypeController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult StudentPhotoTypeOperation([FromBody] M39StudentPhotoType model)
        {
            try
            {
                /* ================= JWT ================= */
                ...
                using (SqlCommand cmd = new SqlCommand(
                    "[dbo].[U77_Pro_M39_StudentPhotoType_Operation]", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@DocumentTypeCode",
                        (object?)model.DocumentTypeCode ?? DBNull.Value);

                    cmd.Parameters.AddWithValue("@DocumentTypeName",
                        (object?)model.DocumentTypeName ?? DBNull.Value);

...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M26IncomeMasterController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M26IncomeMasterController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult IncomeMasterOperation([FromBody] M26IncomeMaster model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M26_IncomeMaster_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@IncomeRange", (object?)model.IncomeRange ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@IncomeCode", (object?)model.IncomeCode ?? DBNull.Value);

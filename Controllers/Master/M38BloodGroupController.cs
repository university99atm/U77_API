...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M38BloodGroupController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M38BloodGroupController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult BloodGroupOperation([FromBody] M38BloodGroup model)
        {
            try
            {
                // 🔐 JWT Claims
                ...
                using (SqlCommand cmd = new SqlCommand(
                    "[dbo].[U77_Pro_M38_BloodGroup_Operation]", con))
                {

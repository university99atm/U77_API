...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M33QualificationController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M33QualificationController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult QualificationOperation([FromBody] M33Qualification model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M33_Qualification_Operation]", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@QualificationName", (object?)model.QualificationName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@QualificationCode", (object?)model.QualificationCode ?? DBNull.Value);

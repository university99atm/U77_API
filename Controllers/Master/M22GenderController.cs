...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M22GenderController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M22GenderController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult GenderOperation([FromBody] M22Gender model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M22_Gender_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@GenderName", (object?)model.GenderName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@GenderCode", (object?)model.GenderCode ?? DBNull.Value);

...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M21SpecialCategoryController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M21SpecialCategoryController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult SpecialCategoryOperation([FromBody] M21SpecialCategory model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M21_SpecialCategory_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@SubCategoryName", (object?)model.SubCategoryName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@SubCategoryCode", (object?)model.SubCategoryCode ?? DBNull.Value);

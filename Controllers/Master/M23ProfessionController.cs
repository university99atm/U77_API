...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M23ProfessionController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M23ProfessionController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult ProfessionOperation([FromBody] M23Profession model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M23_Profession_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@ProfessionName", (object?)model.ProfessionName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@ProfessionCode", (object?)model.ProfessionCode ?? DBNull.Value);

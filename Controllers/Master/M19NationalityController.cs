...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M19NationalityController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M19NationalityController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult NationalityOperation([FromBody] M19Nationality model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M19_Nationality_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@NationalityName", (object?)model.NationalityName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@NationalityCode", (object?)model.NationalityCode ?? DBNull.Value);

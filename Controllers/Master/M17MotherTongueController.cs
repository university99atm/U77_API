...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M17MotherTongueController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M17MotherTongueController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult MotherTongueOperation([FromBody] M17MotherTongue model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M17_MotherTongue_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@MotherTongueName", (object?)model.MotherTongueName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@MotherTongueCode", (object?)model.MotherTongueCode ?? DBNull.Value);

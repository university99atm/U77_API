...
namespace atmglobalapi.Controllers.Master
{
    ...
    [ApiController]
    [Authorize]
    public class M32BoardUniversityController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public M32BoardUniversityController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("operation")]
        public IActionResult BoardUniversityOperation([FromBody] M32BoardUniversity model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[U77_Pro_M32_BoardUniversity_Operation]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@Type", model.Type);
                    cmd.Parameters.AddWithValue("@Id", (object?)model.Id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@BoardName", (object?)model.BoardName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@BoardCode", (object?)model.BoardCode ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@BoardType", (object?)model.BoardType ?? DBNull.Value);
